...
3. **Google Search Console:** propiedad de tipo *Dominio* `alpacaatlas.com` (cubre `www` y el dominio sin www), verificada con un registro TXT `google-site-verification=…` en Netlify DNS. El mismo código está en `GOOGLE_SITE_VERIFICATION` como metaetiqueta de respaldo. Después, enviar `https://alpacaatlas.com/sitemap.xml`.
...
5. **Ramas:** `main` guarda el código y las tareas programadas (GitHub solo las ejecuta desde la rama por defecto); Netlify publica la rama `claude/alpaca-catalog-agent-rhyuwz`, que es donde la actualización diaria deja el catálogo (`PRODUCTION_BRANCH` en el workflow). Si en Netlify se cambia la rama de producción a `main`, hay que cambiar también esa variable.
