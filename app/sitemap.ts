...
import { US_PRODUCTS, isDuplicate, productPath } from "@/lib/catalog.ts";
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [
    // Sin barra final: igual que la URL canónica de la portada.
    { url: SITE.url, lastModified: catalogDate, changeFrequency: "daily", priority: 1 },
    ...COLLECTIONS.map((c) => ({
    ...
    ...US_PRODUCTS.filter((p) => !isDuplicate(p)).map((p) => ({
